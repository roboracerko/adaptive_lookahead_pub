...
```bash
$ cd f1tenth_gym_ros
$ docker build -t f1tenth_gym_ros:humble -f Dockerfile.humble .
```
...
```bash
$ rocker --nvidia --x11 --volume .:/sim_ws/src/f1tenth_gym_ros -- f1tenth_gym_ros:humble
```
...
```bash
docker exec -it f1tenth_gym_ros-sim-humble-1 /bin/bash
```
